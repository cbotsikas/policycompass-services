...
def rebuild_index_itemtype(itemtype):
    ...
    indexing_log = 'Indexing service of ' + itemtype + ' started at ' + str(
        datetime.datetime.now()) + '.\n'
    # Clear item type on Elastic Search Server
    indexing_log = indexing_log + '\n' + 'Deleting existing indexes: ' + requests.delete(
        settings.ELASTICSEARCH_URL + itemtype).text + '\n'
    ...
    while page != 'None':
        ...
        for item_to_index in data["results"]:
            indexing_log = indexing_log + index_item(itemtype,
                                                     item_to_index) + '\n'
...
def init_Index_Mappings(itemtype):
    ...
    response = requests.put(
        settings.ELASTICSEARCH_URL + '_mapping/' + itemtype, data=mapping)
